...
// 수정해야함
// 아이템, 스테이지 // json파일에 기존값 합쳐버림 //크기에서 1.5 나눠주는것은 다른식에서 적용하기기
...
function reset() {
  ...
  itemController.reset(); //아이템 리셋
  sendEvent(2, { timestamp: Date.now() }); //핸들러 2
}
...
function gameLoop(currentTime) {
  ...
  if (!gameover && cactiController.collideWith(player)) {
    gameover = true;
    score.setHighScore();
    sendEvent(3, { timestamp: Date.now(), score: Math.floor(score.score) }); //3번 핸들러로 보내주기
    setupGameReset();
  }
